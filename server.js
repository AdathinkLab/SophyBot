...
app.post("/", async function (req, res) {
	...
	switch (intent) {
		case "PedirDistrito": GPS(FBTools, entities); break;
		case "PedirCiudadDistrito": GPS(FBTools, entities); break;
		case "GPS ON":askLocation(FBTools) ; break;
		case "PedirDepartamentos": GPS(FBTools, entities); break;
		default: FBTools.sendMessage(data.message[0].valor)
	}
})

// Funcion para cosnulta a la base de datos
function GPS(FBTools, entities) {
	SophyDB.connect(async (client) => {
		const places = await SophyDB.selectAllPlace()
		let arrayCarrusel = []
